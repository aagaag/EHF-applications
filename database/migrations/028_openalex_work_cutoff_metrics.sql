...
BEGIN
    ...
    SELECT
        ...
        NULL AS TotalCitations,
        NULL AS Orcid,
        NULL AS GoogleScholarCitationCount,
        NULL AS IdentityCertainty,
        openalex.CitationCount AS VerifiedCitationCount,
        N''OPENALEX_WORK_CUTOFF'' AS VerifiedCitationSource,
        NULL AS VerifiedCitationProfileUrl,
        openalex.PublishedPaperCount AS ValidatedPublishedPaperCount,
    ...
    OUTER APPLY (
        SELECT
            SUM(CASE WHEN latest.CitationStatus=''OBSERVED'' THEN latest.CitationCount ELSE CONVERT(bigint,0) END) AS CitationCount,
        ...
        OUTER APPLY (SELECT TOP (1) CitationCount,CitationStatus FROM dbo.PublicationCitationObservation WHERE ApplicationPublicationId=publication_row.ApplicationPublicationId AND SourceCode=''OPENALEX'' ORDER BY CASE WHEN ObservedAtUtc IS NULL THEN 1 ELSE 0 END,ObservedAtUtc DESC,RecordedAtUtc DESC,PublicationCitationObservationId DESC) AS latest
...
EXEC(N'
CREATE PROCEDURE dbo.GetInternalApplicantMetricDetail
...
BEGIN
    ...
    SELECT publication_row.Title,publication_row.JournalText,publication_row.PublicationYear,
           ...
           latest.CitationCount,latest.EvidenceJson
    FROM dbo.ApplicationPublication AS publication_row
    OUTER APPLY (SELECT TOP (1) ReviewDisposition FROM dbo.ApplicationPublicationReview WHERE ApplicationPublicationId=publication_row.ApplicationPublicationId ORDER BY RecordedAtUtc DESC,ApplicationPublicationReviewId DESC) AS latest_review
