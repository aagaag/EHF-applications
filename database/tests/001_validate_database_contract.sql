...
IF (SELECT COUNT_BIG(*) FROM dbo.SchemaMigration) <> 34
    THROW 51103, 'Exactly thirty-four migrations must be recorded.', 1;
IF EXISTS
(
    SELECT 1
...
WHERE MigrationCount = 34 AND CurrentVersion = 34
)
    THROW 51105, 'SchemaVersion does not report version 34.', 1;
