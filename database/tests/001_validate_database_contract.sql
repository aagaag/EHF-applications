...
IF (SELECT COUNT_BIG(*) FROM dbo.SchemaMigration) <> 6
    THROW 51103, 'Exactly six migrations must be recorded.', 1;
IF EXISTS
(
    SELECT 1
...
(
    ...
    WHERE MigrationCount = 6 AND CurrentVersion = 6
)
    THROW 51105, 'SchemaVersion does not report version 6.', 1;
