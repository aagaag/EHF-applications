...
IF (SELECT COUNT_BIG(*) FROM dbo.SchemaMigration) <> 25
    THROW 51103, 'Exactly twenty-five migrations must be recorded.', 1;
IF EXISTS
(
    SELECT 1
...
WHERE MigrationCount = 25 AND CurrentVersion = 25
)
    THROW 51105, 'SchemaVersion does not report version 25.', 1;
