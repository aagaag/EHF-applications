...
IF (SELECT COUNT_BIG(*) FROM dbo.SchemaMigration) <> 37
    THROW 51103, 'Exactly thirty-seven migrations must be recorded.', 1;
IF EXISTS
(
    SELECT 1
...
WHERE MigrationCount = 37 AND CurrentVersion = 37
)
    THROW 51105, 'SchemaVersion does not report version 37.', 1;
