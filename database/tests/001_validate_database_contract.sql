...
IF (SELECT COUNT_BIG(*) FROM dbo.SchemaMigration) <> 22
    THROW 51103, 'Exactly twenty-two migrations must be recorded.', 1;
IF EXISTS
(
    SELECT 1
...
WHERE MigrationCount = 22 AND CurrentVersion = 22
)
    THROW 51105, 'SchemaVersion does not report version 22.', 1;
