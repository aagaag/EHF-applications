...
IF (SELECT COUNT_BIG(*) FROM dbo.SchemaMigration) <> 4
    THROW 51103, 'Exactly four migrations must be recorded.', 1;
IF EXISTS
(
    SELECT 1
...
(
    ...
    WHERE MigrationCount = 4 AND CurrentVersion = 4
)
    THROW 51105, 'SchemaVersion does not report version 4.', 1;
