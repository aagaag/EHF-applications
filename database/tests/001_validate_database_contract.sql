...
IF (SELECT COUNT_BIG(*) FROM dbo.SchemaMigration) <> 44
    THROW 51103, 'Exactly forty-three migrations must be recorded.', 1;
IF EXISTS
(
    SELECT 1
...
WHERE MigrationCount = 44 AND CurrentVersion = 44
)
    THROW 51105, 'SchemaVersion does not report version 44.', 1;
