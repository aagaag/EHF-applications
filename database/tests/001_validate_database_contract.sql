...
IF (SELECT COUNT_BIG(*) FROM dbo.SchemaMigration) <> 9
    THROW 51103, 'Exactly nine migrations must be recorded.', 1;
IF EXISTS
(
    SELECT 1
...
(
    ...
    WHERE MigrationCount = 9 AND CurrentVersion = 9
)
    THROW 51105, 'SchemaVersion does not report version 9.', 1;
