...
IF (SELECT COUNT_BIG(*) FROM dbo.SchemaMigration) <> 26
    THROW 51103, 'Exactly twenty-six migrations must be recorded.', 1;
IF EXISTS
(
    SELECT 1
...
WHERE MigrationCount = 26 AND CurrentVersion = 26
)
    THROW 51105, 'SchemaVersion does not report version 26.', 1;
