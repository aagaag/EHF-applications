...
IF (SELECT COUNT_BIG(*) FROM dbo.SchemaMigration) <> 24
    THROW 51103, 'Exactly twenty-four migrations must be recorded.', 1;
IF EXISTS
(
    SELECT 1
...
WHERE MigrationCount = 24 AND CurrentVersion = 24
)
    THROW 51105, 'SchemaVersion does not report version 24.', 1;
