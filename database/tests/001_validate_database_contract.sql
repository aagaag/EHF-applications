...
IF (SELECT COUNT_BIG(*) FROM dbo.SchemaMigration) <> 23
    THROW 51103, 'Exactly twenty-three migrations must be recorded.', 1;
IF EXISTS
(
    SELECT 1
...
WHERE MigrationCount = 23 AND CurrentVersion = 23
)
    THROW 51105, 'SchemaVersion does not report version 23.', 1;
