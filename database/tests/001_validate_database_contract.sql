...
IF (SELECT COUNT_BIG(*) FROM dbo.SchemaMigration) <> 5
    THROW 51103, 'Exactly five migrations must be recorded.', 1;
IF EXISTS
(
    SELECT 1
...
(
    ...
    WHERE MigrationCount = 5 AND CurrentVersion = 5
)
    THROW 51105, 'SchemaVersion does not report version 5.', 1;
