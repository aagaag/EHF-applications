...
IF (SELECT COUNT_BIG(*) FROM dbo.SchemaMigration) <> 36
    THROW 51103, 'Exactly thirty-six migrations must be recorded.', 1;
IF EXISTS
(
    SELECT 1
...
WHERE MigrationCount = 36 AND CurrentVersion = 36
)
    THROW 51105, 'SchemaVersion does not report version 36.', 1;
