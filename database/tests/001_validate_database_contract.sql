...
IF (SELECT COUNT_BIG(*) FROM dbo.SchemaMigration) <> 3
    THROW 51103, 'Exactly three migrations must be recorded.', 1;
IF EXISTS
(
    SELECT 1
...
(
    ...
    WHERE MigrationCount = 3 AND CurrentVersion = 3
)
    THROW 51105, 'SchemaVersion does not report version 3.', 1;
