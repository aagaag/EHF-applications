...
IF @MetricsDefinition NOT LIKE N'%COUNT(CASE WHEN latest.CitationStatus=''OBSERVED'' THEN 1 END)=0%'
   AND @MetricsDefinition NOT LIKE N'%CitationMetricCutoffRun%'
    THROW 52920, 'Internal metrics do not distinguish absent OpenAlex observations from zero citations.', 1;
IF @MetricsDefinition NOT LIKE N'%THEN NULL%'
