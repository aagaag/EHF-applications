...
if (($validationFiles.Name -join ',') -ne
    ...
    throw 'The isolated test requires exactly validators 001 through 031.'
}
...
try {
    ...
    if (($firstRun -join "`n") -notmatch 'Applied 31 migration\(s\)\.') {
        throw 'The first migration run did not apply exactly thirty-one migrations.'
    }
