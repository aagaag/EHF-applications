...
if (($migrationFiles.Name -join ',') -ne
    '001_database_contract.sql,002_application_core.sql,003_audit_and_preferences.sql') {
    throw 'The isolated test requires exactly migrations 001 through 003.'
}
if (($validationFiles.Name -join ',') -ne
    '001_validate_database_contract.sql,002_validate_application_core.sql,003_validate_audit_and_preferences.sql') {
    throw 'The isolated test requires exactly validators 001 through 003.'
}
...
try {
    ...
    if (($firstRun -join "`n") -notmatch 'Applied 3 migration\(s\)\.') {
        throw 'The first migration run did not apply exactly three migrations.'
    }
