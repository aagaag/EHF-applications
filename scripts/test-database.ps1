...
if (($migrationFiles.Name -join ',') -ne
    '001_database_contract.sql,002_application_core.sql,003_audit_and_preferences.sql,004_audit_and_preference_hardening.sql,005_application_permissions.sql,006_user_preference_read.sql') {
    throw 'The isolated test requires exactly migrations 001 through 006.'
}
if (($validationFiles.Name -join ',') -ne
    '001_validate_database_contract.sql,002_validate_application_core.sql,003_validate_audit_and_preferences.sql,004_validate_audit_and_preference_hardening.sql,005_validate_application_permissions.sql,006_validate_user_preference_read.sql') {
    throw 'The isolated test requires exactly validators 001 through 006.'
}
...
try {
    ...
    if (($firstRun -join "`n") -notmatch 'Applied 6 migration\(s\)\.') {
        throw 'The first migration run did not apply exactly six migrations.'
    }
