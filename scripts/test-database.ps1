...
if (($validationFiles.Name -join ',') -ne
    ...
    throw 'The isolated test requires exactly validators 001 through 024.'
}
...
try {
    ...
    if (($firstRun -join "`n") -notmatch 'Applied 24 migration\(s\)\.') {
        throw 'The first migration run did not apply exactly twenty-four migrations.'
    }
