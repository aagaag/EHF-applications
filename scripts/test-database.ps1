...
if (($migrationFiles.Name -join ',') -ne
    ...
    throw 'The isolated test requires exactly migrations 001 through 026.'
}
if (($validationFiles.Name -join ',') -ne
    ...
    throw 'The isolated test requires exactly validators 001 through 026.'
}
...
try {
    ...
    if (($firstRun -join "`n") -notmatch 'Applied 26 migration\(s\)\.') {
        throw 'The first migration run did not apply exactly twenty-six migrations.'
    }
