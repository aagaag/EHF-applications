...
if (($validationFiles.Name -join ',') -ne
    ...
    throw 'The isolated test requires exactly validators 001 through 029.'
}
...
try {
    ...
    if (($firstRun -join "`n") -notmatch 'Applied 29 migration\(s\)\.') {
        throw 'The first migration run did not apply exactly twenty-nine migrations.'
    }
