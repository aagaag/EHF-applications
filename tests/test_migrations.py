...
def test_original_003_prefix_upgrades_through_applicant_publication_preview() -> None:
    ...
    assert applied == 43
    assert sorted(connection.records) == list(range(1, 47))
...
def test_fresh_repository_run_applies_all_thirty_eight_migrations() -> None:
    ...
    assert [migration.version for migration in migrations] == list(range(1, 47))
    assert applied == 46
    assert connection.records == {
...
def test_academic_age_recovery_migration_is_ordered_and_preserves_the_metrics_boundary() -> None:
    ...
    assert [migration.path.name for migration in migrations[-20:]] == [
        ...
        "044_call_navigation_preferences.sql",
...
def test_isolated_status_probe_creates_a_call_owned_applicant() -> None:
    helper = (ROOT / "infra" / "sql-principal.py").read_text(encoding="utf-8")
    assert "INSERT dbo.Applicant (ApplicantId,FellowshipCallId,LegalGivenNames,LegalFamilyName) VALUES (@ApplicantId,@CallId" in helper
...
def test_every_table_has_a_primary_key_and_database_generated_utc_timestamp() -> None:
    ...
    assert set(blocks) == {
        ...
        "ApplicantCitationProfileObservation",
            ...
            "InternalReviewArtifactProvenance",
            "ShortlistTrustee",
    ...
    for table_name, block in blocks.items():
        ...
        assert re.search(
            r"\b(?:Applied|Created|Recorded|Occurred|Started|Observed|Decided|Requested|Linked|Reviewed)AtUtc\s+datetime2\(7\)",
            block,
            flags=re.IGNORECASE,
        ), table_name
...
def test_database_script_requires_and_applies_019() -> None:
    ...
    assert "022_applicant_publication_preview.sql" in script
    assert "022_validate_applicant_publication_preview.sql" in script
    assert "023_open_citation_sources.sql" in script
    assert "023_validate_open_citation_sources.sql" in script
    assert "024_applicant_citation_profiles.sql" in script
    assert "024_validate_applicant_citation_profiles.sql" in script
    assert "025_publication_review_workflow.sql" in script
    assert "025_validate_publication_review_workflow.sql" in script
    assert "026_internal_document_access.sql" in script
    ...
    assert "044_call_navigation_preferences.sql" in script
    assert "044_validate_call_navigation_preferences.sql" in script
    assert "045_fix_call_navigation_preference_insert.sql" in script
    assert "045_validate_call_navigation_preference_insert.sql" in script
    assert "046_fix_call_navigation_audit_payload.sql" in script
...
def test_database_contract_validator_reports_version_thirty_eight() -> None:
    ...
    assert "COUNT_BIG(*) FROM dbo.SchemaMigration) <> 46" in validator
    assert "WHERE MigrationCount = 46 AND CurrentVersion = 46" in validator


def test_multi_call_foundation_exposes_call_ownership_and_authorized_catalog_contract() -> None:
    """Break caught: calls could remain a single implicit EHF-2026 namespace."""
    migration = (MIGRATION_DIRECTORY / "040_multi_call_foundation.sql").read_text(
        encoding="utf-8"
    )
    compatibility_migration = (
        MIGRATION_DIRECTORY / "042_multi_call_compatibility.sql"
    ).read_text(encoding="utf-8")
    ...
    assert "DF_FellowshipCall_PublicSlug" in compatibility_migration
    assert "ALTER PROCEDURE dbo.CreateSyntheticApplicantWorkspace" in compatibility_migration
    assert "ON UPDATE CASCADE" in compatibility_migration
    for fragment in (
...
def test_applicant_journal_metric_migration_projects_latest_openalex_evidence_safely() -> None:
    migration = (
        MIGRATION_DIRECTORY / "034_applicant_journal_metrics.sql"
    ).read_text(encoding="utf-8")
    validator = (
        VALIDATION_DIRECTORY / "034_validate_applicant_journal_metrics.sql"
    ).read_text(encoding="utf-8")
