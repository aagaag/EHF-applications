...
def test_original_003_prefix_upgrades_through_applicant_publication_preview() -> None:
    ...
    assert applied == 41
    assert sorted(connection.records) == list(range(1, 45))
...
def test_fresh_repository_run_applies_all_thirty_eight_migrations() -> None:
    ...
    assert [migration.version for migration in migrations] == list(range(1, 45))
    assert applied == 44
    assert connection.records == {
...
def test_database_script_requires_and_applies_019() -> None:
    ...
    assert "Applied 44 migration\\(s\\)\\." in script
...
def test_database_contract_validator_reports_version_thirty_eight() -> None:
    ...
    assert "COUNT_BIG(*) FROM dbo.SchemaMigration) <> 44" in validator
    assert "WHERE MigrationCount = 44 AND CurrentVersion = 44" in validator
