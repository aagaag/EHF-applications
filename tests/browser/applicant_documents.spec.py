...
def test_documents_page_explains_controlled_pdf_slots_without_confidential_status() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            expect(page.get_by_role("heading", name="Your application documents")).to_be_visible()
            expect(page.get_by_text("PDF files only", exact=False)).to_be_visible()
...
def test_available_document_has_view_download_and_complete_package_controls() -> None:
    """Break caught: applicants could not study originals or their complete PDF package."""
    pytest.importorskip("playwright.sync_api")
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            view = page.get_by_role("link", name="View Curriculum vitae")
            download = page.get_by_role("link", name="Download Curriculum vitae")
            view_package = page.get_by_role(
                "link", name="View complete application PDF"
            )
            download_package = page.get_by_role(
                "link", name="Download complete application PDF"
            )
            assert view.get_attribute("href") == f"/api/applicant/documents/{slot_id}/view"
            assert view.get_attribute("target") == "_blank"
            assert download.get_attribute("href") == f"/api/applicant/documents/{slot_id}/download"
            assert view_package.get_attribute("href") == "/api/applicant/documents/package/view"
            assert view_package.get_attribute("target") == "_blank"
            assert download_package.get_attribute("href") == "/api/applicant/documents/package/download"
        finally:
            browser.close()
